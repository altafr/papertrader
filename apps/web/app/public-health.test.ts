...
describe("parsePublicHealth", () => {
  it("keeps the bounded non-sensitive heartbeat fields", () => {
    expect(parsePublicHealth({ status: "healthy", operatingMode: "paper_autopilot", release: "abc123", researchSchedule: { status: "scheduled", nextRunAt: "2026-08-29T20:00:00Z" }, positionManagement: { readiness: "ready" }, marketStream: { freshness: "fresh", lastMessageAt: "2026-08-29T19:59:30Z", status: "connected" }, accountEquity: "secret" })).toEqual({ status: "healthy", operatingMode: "paper_autopilot", release: "abc123", researchSchedule: { status: "scheduled", nextRunAt: "2026-08-29T20:00:00Z" }, positionManagement: { readiness: "ready" }, marketStream: { freshness: "fresh", lastMessageAt: "2026-08-29T19:59:30Z", status: "connected" } });
  });
