export type PublicHealth = {
  ...
  readonly positionManagement?: { readonly readiness?: string; readonly status?: string; readonly unmanagedCount?: number };
...
export function parsePublicHealth(value: unknown): PublicHealth | undefined {
  ...
  return {
    ...
    ...(positions ? { positionManagement: { ...(optionalString(positions.readiness) ? { readiness: positions.readiness as string } : {}), ...(optionalString(positions.status) ? { status: positions.status as string } : {}), ...(typeof positions.unmanagedCount === "number" && Number.isSafeInteger(positions.unmanagedCount) && positions.unmanagedCount >= 0 && positions.unmanagedCount <= 100 ? { unmanagedCount: positions.unmanagedCount } : {}) } } : {}),
