export type PublicHealth = {
  ...
  readonly marketStream?: { readonly freshness?: "fresh" | "stale" | "unknown"; readonly lastMessageAt?: string; readonly status?: string };
};
...
export function parsePublicHealth(value: unknown): PublicHealth | undefined {
  ...
  return {
    ...
    ...(stream ? { marketStream: { ...(optionalString(stream.freshness) && (["fresh", "stale", "unknown"] as const).includes(stream.freshness as "fresh" | "stale" | "unknown") ? { freshness: stream.freshness as "fresh" | "stale" | "unknown" } : {}), ...(optionalString(stream.lastMessageAt) ? { lastMessageAt: stream.lastMessageAt as string } : {}), ...(optionalString(stream.status) ? { status: stream.status as string } : {}) } } : {}),
  };
}
