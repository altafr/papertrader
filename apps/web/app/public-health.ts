export type PublicHealth = {
  ...
  readonly researchSchedule?: { readonly lastCatchupAt?: string; readonly lastCatchupJobId?: string; readonly lastCatchupStatus?: "queued" | "rejected"; readonly status?: string; readonly nextRunAt?: string };
...
export function parsePublicHealth(value: unknown): PublicHealth | undefined {
  ...
  return {
    ...
    ...(research ? { researchSchedule: { ...(optionalString(research.lastCatchupAt) ? { lastCatchupAt: research.lastCatchupAt as string } : {}), ...(optionalString(research.lastCatchupJobId) ? { lastCatchupJobId: research.lastCatchupJobId as string } : {}), ...((research.lastCatchupStatus === "queued" || research.lastCatchupStatus === "rejected") ? { lastCatchupStatus: research.lastCatchupStatus } : {}), ...(optionalString(research.status) ? { status: research.status as string } : {}), ...(optionalString(research.nextRunAt) ? { nextRunAt: research.nextRunAt as string } : {}) } } : {}),
