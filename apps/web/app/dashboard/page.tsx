  ...
  return (
    <article className="card full-width agent-runs-card" aria-label="Agent run health">
      ...
      {!runs ? <p className="empty-state">Authenticated agent-run metadata is currently unavailable.</p> : runs.length === 0 ? <p className="empty-state">No agent runs have been recorded.</p> : (
        <div className="agent-runs-list">{runs.slice(0, 8).map((run) => <div className="agent-run-row" key={run.runId}><div><strong>{run.agentType}</strong><span>{run.task} · {formatUtc(run.createdAt)}</span>{run.artifact?.rationale && <small>{run.artifact.rationale}</small>}<small className="provenance">{run.artifact?.confidence ? `Confidence ${run.artifact.confidence}` : "Confidence not reported"}{run.artifact?.type ? ` · ${run.artifact.type}` : ""}{run.artifact?.evidenceRefs?.length ? ` · ${run.artifact.evidenceRefs.length} evidence refs` : " · No evidence refs"}</small>{run.artifact?.evidenceRefs?.length ? <small className="evidence-refs">Evidence: {run.artifact.evidenceRefs.join(" · ")}</small> : null}</div><span className={`state-badge ${run.status === "succeeded" ? "fresh" : run.status === "failed" ? "degraded" : "delayed"}`}>{run.status}</span></div>)}</div>
      )}
