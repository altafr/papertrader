...
describe("dashboard state", () => {
  ...
  it("accepts only the redacted operations-health contract", () => {
    const health = parseOperationsHealth({
      ...
      runtime: {
        ...
        riskCycle: { approved: 1, decisions: 3, latestAt: "2026-08-23T00:00:05.000Z" },
    ...
    expect(parseOperationsHealth({
      ...
      runtime: {
        ...
        paperAutopilotOrderSubmissionEnabled: false,
        paperAutopilotOrderSubmissionApprovalReferencePresent: true,
        paperBaseline: { current: "unavailable", initial: "unavailable", status: "blocked" },
        ...
        riskCycle: { approved: 0, decisions: 0 },
        scheduler: { activationApprovalReferencePresent: true, cron: "0 0 * * *", enabled: false, status: "disabled", timezone: "UTC" },
        telegramAlerts: { deliveryVerification: "unverified", enabled: false, status: "disabled" },
        telegramAlertTest: { approvalReferencePresent: false, status: "blocked" },
      },
  ...
  it("preserves the selected performance range during audit navigation", () => {
    ...
    expect(buildDashboardHistoryParams(1, "7d", "2026-08-20", "2026-08-26").get("from")).toBe("2026-08-20");
  });
