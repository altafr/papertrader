import { createPaperExitOrderSubmitter, type PaperExitOrderSubmitter } from "@momentum/alpaca";
...
export async function runPaperPositionManagementOnce(input: {
  ...
  for (const position of input.positions) {
    ...
    await input.submitter.submitExit({ assetClass: position.assetClass, clientOrderId: `${position.intentId}-exit-${decision.reason}`, decision, quantity: position.quantity, timeInForce: "day", type: "market" });
    submitted += 1;
  }
