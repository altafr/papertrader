...
export function buildPositionExitDecisionLog(input: { readonly currentPrice?: string; readonly entryPrice?: string; readonly plannedStopPrice?: string; readonly effectiveStopPrice?: string; readonly plannedTargetPrice?: string; readonly reason?: string; readonly shouldExit: boolean; readonly strategyKey?: string; readonly strategyVersion?: string; readonly submitted?: boolean; readonly symbol: string; readonly timeStopAt?: string }) {
  return { event: "position_exit_decision", ...(input.currentPrice ? { currentPrice: input.currentPrice } : {}), ...(input.entryPrice ? { entryPrice: input.entryPrice } : {}), ...(input.plannedStopPrice ? { plannedStopPrice: input.plannedStopPrice } : {}), ...(input.effectiveStopPrice ? { effectiveStopPrice: input.effectiveStopPrice } : {}), ...(input.plannedTargetPrice ? { plannedTargetPrice: input.plannedTargetPrice } : {}), reason: input.reason ?? null, shouldExit: input.shouldExit, ...(input.strategyKey ? { strategyKey: input.strategyKey } : {}), ...(input.strategyVersion ? { strategyVersion: input.strategyVersion } : {}), submitted: input.submitted ?? false, symbol: input.symbol, ...(input.timeStopAt ? { timeStopAt: input.timeStopAt } : {}) } as const;
}
