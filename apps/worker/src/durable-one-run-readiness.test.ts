...
describe("durable one-run readiness", () => {
  ...
  it("reports ready only for a fully gated temporary one-run", () => {
    expect(getDurableOneRunReadiness({ ALPACA_API_KEY: "key", ALPACA_SECRET_KEY: "secret", ALPACA_PAPER_TRADE: "true", BROKER_CONNECTION_ENABLED: "true", DAILY_PREPARATION_HANDLER_ENABLED: "true", DATABASE_URL: "postgres://redacted", DURABLE_ONE_RUN_ID: "run-1", DURABLE_SCHEDULER_APPROVAL_REFERENCE: "ticket-123", DURABLE_SCHEDULER_ONCE: "true", PAPER_AUTOPILOT_ENABLED: "false", TRADING_MODE: "paper" })).toMatchObject({ approvalReferencePresent: true, runIdPresent: true, blockedReasons: [], status: "ready" });
  });
