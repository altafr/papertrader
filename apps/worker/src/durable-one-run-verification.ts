...
export interface DurableOneRunVerification {
  ...
  readonly provenance: { readonly approvalReference: string; readonly runId: string };
...
export function assessDurableOneRunVerification(input: { readonly queues: DurableQueueInspection; readonly capturedAt?: Date | string; readonly now?: Date; readonly approvalReference?: string; readonly runId?: string }): DurableOneRunVerification {
  ...
  return {
    blockedReasons,
    queues: input.queues,
    provenance: { approvalReference: input.approvalReference ?? "", runId: input.runId ?? "" },
