...
try {
  ...
  console.log(JSON.stringify({ backfillInputs: EXIT_PLAN_BACKFILL_INPUTS, positions: report, reviewRequired: report.filter((row) => row.status === "review_required").length, status: "exit_plan_review" }));
