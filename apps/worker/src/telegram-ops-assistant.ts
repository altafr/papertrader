...
export async function buildTelegramOpsAssistantReply(question: string, data: TelegramOpsAssistantData): Promise<string> {
  ...
  if (asksExitPlan) {
    ...
    const status = model.positions.map((position) => {
      ...
      if (plan) return `${position.symbol}: managed (active stop ${display(plan.trailingStopPrice ?? plan.plannedStopPrice)}, planned stop ${display(plan.plannedStopPrice)}, ${plan.plannedTargetPrice ? `target ${display(plan.plannedTargetPrice)}` : `time stop ${utc(plan.timeStopAt!)}`})`;
