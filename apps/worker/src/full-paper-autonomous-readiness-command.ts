...
try {
  ...
  const readiness = combineFullPaperAutonomousReadiness({
    ...
    // The notification package deliberately reports delivery as unverified until an approved test has been run.
    alerts: { enabled: telegram.checks.enabled, configured: telegram.checks.botTokenConfigured && telegram.checks.chatIdConfigured, deliveryVerified: false },
    performance,
  });
