...
export function getPaperAutopilotQuantity(assetClass: ResearchWatchlistCandidate["assetClass"], environment: NodeJS.ProcessEnv = process.env): string {
  const configured = (assetClass === "crypto" ? environment.PAPER_AUTOPILOT_CRYPTO_QUANTITY : environment.PAPER_AUTOPILOT_STOCK_QUANTITY)?.trim();
