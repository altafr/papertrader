...
async function readOperationsHealth(request: IncomingMessage) {
  ...
  try {
    ...
    const riskCycleResult = await migrationDatabase.pool.query<{ readonly latest_at: Date | null; readonly decisions: number; readonly approved: number }>("SELECT MAX(COALESCE(updated_at, created_at)) FILTER (WHERE risk_decision IS NOT NULL) AS latest_at, COUNT(*) FILTER (WHERE risk_decision IS NOT NULL)::int AS decisions, COUNT(*) FILTER (WHERE risk_decision->>'approvalStatus' = 'approved' OR status = 'risk_dry_run_approved')::int AS approved FROM paper_order_submissions WHERE COALESCE(updated_at, created_at) >= NOW() - INTERVAL '7 days'");
