...
async function readOperatorOverview(request: IncomingMessage) {
  ...
  try {
    ...
    const [agents, filteredTrades, submissions, lifecycle, schedules, agentCount, filteredTradeCount, submissionCount, lifecycleCount, scheduleCount] = await Promise.all([
    ...
    return {
      body: {
        ...
        auditTimeline,
        history: { from: history.from, hasNext: [agents, filteredTrades, submissions, lifecycle, schedules].some((result) => result.rows.length === history.limit), ...(typeof auditTimeline[0]?.capturedAt === "string" ? { latestCapturedAt: auditTimeline[0].capturedAt } : {}), limit: history.limit, page: history.page, to: history.to, totals: { agents: Number(agentCount.rows[0]?.total ?? 0), filteredTrades: Number(filteredTradeCount.rows[0]?.total ?? 0), submissions: Number(submissionCount.rows[0]?.total ?? 0), lifecycle: Number(lifecycleCount.rows[0]?.total ?? 0), schedules: Number(scheduleCount.rows[0]?.total ?? 0) } },
      },
      status: 200,
    } as const;
  } finally {
