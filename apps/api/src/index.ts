...
async function readReadModelCsv(request: IncomingMessage) {
  ...
  const header = ["recordType", "recordId", "symbol", "assetClass", "side", "type", "status", "quantity", "filledQuantity", "averageEntryPrice", "marketValue", "pnl", "price", "capturedAt", "submittedAt", "updatedAt", "exitPlanStatus", "strategyKey", "strategyVersion", "plannedStopPrice", "plannedTargetPrice", "timeStopAt", "positionOpenedAt"];
  const rows = [
    ["account_snapshot", model.snapshot.accountSnapshotId, "", "", "", "", model.snapshot.status, "", "", "", "", "", "", model.freshness.capturedAt, "", "", "", "", "", "", "", ""],
    ...model.positions.map((position) => ["position", position.symbol, position.symbol, position.assetClass, "", "", "open", position.quantity, "", position.averageEntryPrice, position.marketValue, position.unrealizedPl, "", model.freshness.capturedAt, "", "", unmanaged.has(`${position.assetClass}:${position.symbol}`) ? "review_required" : "active", position.strategyKey ?? "", position.strategyVersion ?? "", position.plannedStopPrice ?? "", position.plannedTargetPrice ?? "", position.timeStopAt ?? "", position.positionOpenedAt ?? ""]),
