...
async function readOperatorOverview(request: IncomingMessage) {
  ...
  try {
    const [agents, filteredTrades, submissions] = await Promise.all([
      ...
      pool.query("SELECT intent_id, approval_id, client_order_id, alpaca_order_id, symbol, asset_class, quantity, filled_quantity, status, created_at, submitted_at, updated_at, market_snapshot FROM paper_order_submissions ORDER BY COALESCE(updated_at, created_at) DESC LIMIT 100"),
    ]);
    return {
      body: {
