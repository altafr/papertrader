...
import { assessReconciliationHealth, assessResearchScheduleActivation, assessSchedulerActivation, assessSchedulerAuditGate, readAuditMigrationReadiness, readSchedulerAuditMigrationReadiness, serializeDurableScheduleRunHealth, serializeRiskCycleSummary } from "./operations-health.js";
...
async function readPersistedModel(request: IncomingMessage) {
  ...
  const positionMetadata = readModelPool
    ? (await readModelPool.query<{ asset_class: string; symbol: string; entry_price: string | null; planned_stop_price: string | null; trailing_stop_price: string | null; planned_target_price: string | null; time_stop_at: Date | null; submitted_at: Date | null; strategy_key: string | null; strategy_version: string | null }>("SELECT DISTINCT ON (s.asset_class, s.symbol) s.asset_class, s.symbol, s.entry_price, s.planned_stop_price, s.trailing_stop_price, s.planned_target_price, s.time_stop_at, s.submitted_at, s.strategy_key, s.strategy_version FROM paper_order_submissions s JOIN positions p ON p.asset_class = s.asset_class AND replace(p.symbol, '/', '') = replace(s.symbol, '/', '') JOIN account_snapshots a ON a.id = p.account_snapshot_id WHERE a.id = (SELECT id FROM account_snapshots ORDER BY captured_at DESC, id DESC LIMIT 1) AND s.entry_price IS NOT NULL AND s.planned_stop_price IS NOT NULL AND s.strategy_key IS NOT NULL AND s.strategy_version IS NOT NULL AND (s.planned_target_price IS NOT NULL OR s.time_stop_at IS NOT NULL) AND (s.risk_decision->>'approvalStatus' = 'approved' OR s.alpaca_order_id IS NOT NULL) ORDER BY s.asset_class, s.symbol, COALESCE(s.updated_at, s.created_at) DESC")).rows.map((row): PositionMetadata => ({
      assetClass: row.asset_class,
      symbol: row.symbol,
      ...(row.entry_price ? { entryPrice: row.entry_price } : {}),
      ...(row.planned_stop_price ? { plannedStopPrice: row.planned_stop_price } : {}),
      ...(row.trailing_stop_price ? { effectiveStopPrice: row.trailing_stop_price } : {}),
      ...(row.planned_target_price ? { plannedTargetPrice: row.planned_target_price } : {}),
      ...(row.time_stop_at ? { timeStopAt: row.time_stop_at.toISOString() } : {}),
  ...
  const unmanagedPositions = readModelPool
    ? (await readModelPool.query<{ symbol: string; asset_class: string; alpaca_order_id: string | null; entry_price: string | null; planned_stop_price: string | null; planned_target_price: string | null; time_stop_at: Date | null; strategy_key: string | null; strategy_version: string | null }>("SELECT p.symbol, p.asset_class, s.alpaca_order_id, s.entry_price, s.planned_stop_price, s.planned_target_price, s.time_stop_at, s.strategy_key, s.strategy_version FROM positions p JOIN account_snapshots a ON a.id = p.account_snapshot_id LEFT JOIN LATERAL (SELECT s.alpaca_order_id, s.entry_price, s.planned_stop_price, s.planned_target_price, s.time_stop_at, s.strategy_key, s.strategy_version FROM paper_order_submissions s WHERE replace(s.symbol, '/', '') = replace(p.symbol, '/', '') AND s.asset_class = p.asset_class AND s.entry_price IS NOT NULL AND s.planned_stop_price IS NOT NULL AND s.strategy_key IS NOT NULL AND s.strategy_version IS NOT NULL AND (s.planned_target_price IS NOT NULL OR s.time_stop_at IS NOT NULL) AND (s.risk_decision->>'approvalStatus' = 'approved' OR s.alpaca_order_id IS NOT NULL) ORDER BY COALESCE(s.updated_at, s.created_at) DESC LIMIT 1) s ON TRUE WHERE a.id = (SELECT id FROM account_snapshots ORDER BY captured_at DESC, id DESC LIMIT 1) AND s.alpaca_order_id IS NULL")).rows.map((row) => ({ assetClass: row.asset_class, symbol: row.symbol, missingFields: getExitPlanMissingFields({ alpacaOrderId: row.alpaca_order_id, entryPrice: row.entry_price, plannedStopPrice: row.planned_stop_price, plannedTargetPrice: row.planned_target_price, timeStopAt: row.time_stop_at, strategyKey: row.strategy_key, strategyVersion: row.strategy_version }) }))
    : [];
...
async function readTelegramMiniApp(request: IncomingMessage) {
  ...
  const projectedPositions = model.positions.map((position) => {
    const metadata = metadataByPosition.get(`${position.assetClass}:${position.symbol.replaceAll('/', '').toUpperCase()}`);
    return metadata ? { ...position, ...(metadata.planned_stop_price ? { plannedStopPrice: metadata.planned_stop_price } : {}), ...(metadata.trailing_stop_price ? { effectiveStopPrice: metadata.trailing_stop_price } : {}), ...(metadata.planned_target_price ? { plannedTargetPrice: metadata.planned_target_price } : {}) } : position;
  });
  const unmanaged = await telegramMiniAppPool.query<{ readonly symbol: string; readonly asset_class: string; readonly alpaca_order_id: string | null; readonly entry_price: string | null; readonly planned_stop_price: string | null; readonly planned_target_price: string | null; readonly time_stop_at: Date | null; readonly strategy_key: string | null; readonly strategy_version: string | null }>("SELECT p.symbol, p.asset_class, s.alpaca_order_id, s.entry_price, s.planned_stop_price, s.planned_target_price, s.time_stop_at, s.strategy_key, s.strategy_version FROM positions p JOIN account_snapshots a ON a.id = p.account_snapshot_id LEFT JOIN LATERAL (SELECT s.alpaca_order_id, s.entry_price, s.planned_stop_price, s.planned_target_price, s.time_stop_at, s.strategy_key, s.strategy_version FROM paper_order_submissions s WHERE replace(s.symbol, '/', '') = replace(p.symbol, '/', '') AND s.asset_class = p.asset_class AND s.entry_price IS NOT NULL AND s.planned_stop_price IS NOT NULL AND s.strategy_key IS NOT NULL AND s.strategy_version IS NOT NULL AND (s.planned_target_price IS NOT NULL OR s.time_stop_at IS NOT NULL) AND (s.risk_decision->>'approvalStatus' = 'approved' OR s.alpaca_order_id IS NOT NULL) ORDER BY COALESCE(s.updated_at, s.created_at) DESC LIMIT 1) s ON TRUE WHERE a.id = (SELECT id FROM account_snapshots ORDER BY captured_at DESC, id DESC LIMIT 1) LIMIT 100");
  ...
  try {
    ...
    body: {
      ...
      runtime: {
        ...
        riskCycle: {
          ...(riskCycle.latestAt ? { latestAt: riskCycle.latestAt } : {}),
...
async function readPaperPerformance(request: IncomingMessage) {
  ...
  try {
    ...
    const daysRemaining = Math.max(0, 30 - consecutiveCalendarDays);
    const estimatedReadyAt = latestCapturedAt && daysRemaining > 0 ? new Date(Date.parse(latestCapturedAt) + daysRemaining * 86_400_000).toISOString() : undefined;
    if (snapshots.length < 2) return { body: { calendarDays: dates.length, consecutiveCalendarDays, daysRemaining, ...(estimatedReadyAt ? { estimatedReadyAt } : {}), performanceRange: requestedRange, snapshotCount: snapshots.length, stability: { blockedReasons: ["minimum_30_consecutive_calendar_days_not_met", "performance_history_insufficient"], status: "blocked" }, status: "insufficient_history" }, status: 200 } as const;
...
async function readOperatorOverview(request: IncomingMessage) {
  ...
  try {
    ...
    const [agents, filteredTrades, submissions, lifecycle, schedules, telegramAlerts, agentCount, filteredTradeCount, submissionCount, lifecycleCount, scheduleCount, telegramAlertCount] = await Promise.all([
      ...
      pool.query("SELECT intent_id, approval_id, client_order_id, alpaca_order_id, symbol, asset_class, quantity, filled_quantity, status, created_at, submitted_at, updated_at, market_snapshot, risk_decision, entry_price, planned_stop_price, planned_target_price, exit_plan_reference, strategy_key, strategy_version FROM paper_order_submissions WHERE ($1::timestamptz IS NULL OR COALESCE(updated_at, created_at) >= $1::timestamptz) AND ($2::timestamptz IS NULL OR COALESCE(updated_at, created_at) <= $2::timestamptz) ORDER BY COALESCE(updated_at, created_at) DESC LIMIT $3 OFFSET $4", queryValues),
...
async function readReadModelCsv(request: IncomingMessage) {
  ...
  const model = result.body.model as {
    ...
    readonly unmanagedPositions?: readonly { readonly assetClass: string; readonly symbol: string; readonly missingFields?: readonly string[] }[];
  };
  const unmanagedByKey = new Map((result.body.unmanagedPositions ?? []).map((position) => [`${position.assetClass}:${position.symbol}`, position.missingFields?.join("|") ?? ""]));
  const header = ["recordType", "recordId", "symbol", "assetClass", "side", "type", "status", "quantity", "filledQuantity", "averageEntryPrice", "marketValue", "pnl", "price", "capturedAt", "submittedAt", "updatedAt", "exitPlanStatus", "exitPlanMissingFields", "strategyKey", "strategyVersion", "plannedStopPrice", "plannedTargetPrice", "timeStopAt", "positionOpenedAt"];
  const rows = [
    ["account_snapshot", model.snapshot.accountSnapshotId, "", "", "", "", model.snapshot.status, "", "", "", "", "", "", model.freshness.capturedAt, "", "", "", "", "", "", "", "", ""],
    ...model.positions.map((position) => ["position", position.symbol, position.symbol, position.assetClass, "", "", "open", position.quantity, "", position.averageEntryPrice, position.marketValue, position.unrealizedPl, "", model.freshness.capturedAt, "", "", unmanagedByKey.has(`${position.assetClass}:${position.symbol}`) ? "review_required" : "active", unmanagedByKey.get(`${position.assetClass}:${position.symbol}`) ?? "", position.strategyKey ?? "", position.strategyVersion ?? "", position.plannedStopPrice ?? "", position.plannedTargetPrice ?? "", position.timeStopAt ?? "", position.positionOpenedAt ?? ""]),
