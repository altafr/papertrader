...
Company, earnings, news, sector, and ticker questions route to a persisted `stock_research` or `crypto_research` agent run. With the optional server-side `FIRECRAWL_API_KEY`, the run transitions through `queued`, `running`, and `succeeded`, persists up to three links as untrusted reference material, and returns them to Telegram. Without the key or when the provider fails, the run is marked failed closed. The route is advisory/read-only, cannot create or modify orders, and never affects trading.
