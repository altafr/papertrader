...
After a Railway rollout, set the repository Actions variable `PAPERTRADER_EXPECTED_RELEASE` to the deployed release identifier. Prefer setting the same bounded `PAPERTRADER_RELEASE` variable on both the API and Worker when Railway does not inject a commit SHA; the scheduled monitor and paper-only CI verifier will then fail closed if either service reports a different release. Leaving the expected-release variable unset preserves compatibility during development.
...
The assistant also supports a read-only Telegram Mini App. Configure `TELEGRAM_MINI_APP_URL` on the Worker (the deployed Vercel `/telegram` URL), `TELEGRAM_MINI_APP_ENABLED=true` and the operator's numeric `TELEGRAM_MINI_APP_USER_ID` on the Railway API, and `TELEGRAM_MINI_APP_ORIGIN` to the exact Vercel origin. Send `/myid` in the authorized chat to receive the numeric user ID needed for setup, then send `/dashboard` to receive an **Open portfolio & alerts** button. Telegram signs the Web App session with the server-side bot token; the API validates that signature and allowlisted user before returning the reconciled paper portfolio and recent alerts. The Mini App has no order, risk, or configuration controls.
See [`docs/telegram-mini-app-runbook.md`](docs/telegram-mini-app-runbook.md) for the activation and verification procedure.

Company, earnings, news, sector, and ticker questions route to a persisted `stock_research` or `crypto_research` agent run. With the optional server-side `FIRECRAWL_API_KEY`, the run transitions through `queued`, `running`, and `succeeded`, persists up to three links as untrusted reference material, and returns them to Telegram. Worker health reports the non-secret Firecrawl configuration status. Without the key or when the provider fails, the run is marked failed closed. The route is advisory/read-only, cannot create or modify orders, and never affects trading.
