...
Company, earnings, news, sector, and ticker questions route to a persisted `stock_research` or `crypto_research` agent run. If the optional server-side `FIRECRAWL_API_KEY` is set on the Railway Worker, the assistant performs a bounded search and returns up to three links as untrusted reference material. The route is advisory/read-only, cannot create or modify orders, and fails closed when web lookup is absent or unavailable.
