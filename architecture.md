...
The continuous crypto scheduler also checks the New York 16:00 weekday close hour (with daylight-saving-aware timezone conversion) and emits the summary from the freshly reconciled account model. The existing durable daily schedule remains a fallback; both paths share the same cooldown and dedupe key.
