...
- **Stage:** Phase 6.845 cross-surface smoke verification; continuous Paper Autopilot remains paper-only and fail-closed when broker permissions or market data are incomplete.
- **Current hosted state:** Worker is deployed in Paper Autopilot with fresh crypto data, scheduled research, deterministic risk/execution gates, two-percent minimum sizing, equity bracket entries, ratcheting position stops, restart-safe synthetic crypto protection explicitly enabled, and the read-only Telegram operations assistant enabled with polling. Runtime, reconciliation, position coverage, and Telegram configuration remain ready; the top-level Worker is healthy. The latest scheduled cycle completed at `2026-09-06T07:15:10Z` with one deterministic risk decision and advanced the next cycle to `07:30 UTC`. The dashboard, API, and Worker health surfaces each returned HTTP `200` in the latest smoke check. The guarded readiness audit contains `15,847` snapshots, with 14 consecutive evidence days and 16 remaining; full readiness remains intentionally blocked until the required 30-day paper history exists.
...
The durable `tech_solver` write path persists the bounded/redacted diagnosis as `last_error`, ensuring request IDs and troubleshooting context survive Worker restarts without storing provider response bodies.

Deployment `6f792520-c8a6-477c-a8ac-c339688c8617` verified this persistence in production; the entitlement case now retains its latest redacted request ID for support escalation.
...
Minute and hourly default lookbacks stay below the 100-bar request cap, preventing Alpaca from returning an older slice that would correctly fail the stale-data gate; explicit ranges remain caller-controlled.
...
The repeatable signed Mini App verifier can optionally fetch Worker health and fails closed if unmanaged-position counts differ across surfaces. It reports only bounded counts/status metadata and never prints credentials.
...
Daily portfolio summaries format equity, cash, and buying power with the same two-decimal operator presentation rule used for P/L and exposure; stored broker precision is preserved.

Full-readiness and Telegram evidence telemetry use a centralized 100,000-row account-snapshot limit, sized for the observed high-frequency reconciliation cadence while remaining bounded for hosted queries.
...
The repository hosted verifier currently passes across Railway Worker/API and Vercel, including paper-mode, risk-gate, freshness, scheduler, position-coverage, kill-switch, release-consistency, and public-surface checks.
...
Scheduled risk-cycle position-coverage checks canonicalize broker symbols before matching complete exit-plan provenance, keeping compact/slash-form crypto identities consistent with the deterministic fail-closed gate.
