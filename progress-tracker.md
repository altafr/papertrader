...
- **Where to resume:** Obtain a separate explicit non-secret approval reference for one guarded paper reconciliation, run the bounded one-run command once, run its read-only verifier, and inspect the authenticated dashboard without enabling recurring scheduling.
...
- **Recommended next prompt:** `Approve one paper reconciliation run with reference <your-ticket>` (use a separate non-secret reference; migration approval does not authorize this broker read/database write).
