...
- **What exists:** Verified hosted foundation, authenticated read-only account/dashboard surfaces, protected market-data and stream boundaries, deterministic strategies/replay/risk/execution contracts, research-agent tooling, guarded paper execution wiring, durable queue and scheduler boundaries, migration `0008` applied, reviewed migration `0009` with exact-target/approval guards, API and dashboard migration-readiness visibility, bounded browser/API reason contracts, and a fresh hosted daily-reconciliation readiness check. Railway API deployment `c4882939-bce4-47b9-8e05-38337a170691` and Vercel preview `dpl_4NrANzRza3rdLjSc86NxuxQnv9gG` are ready. Migration `0009` remains unapplied; broker access, recurring scheduler, research scheduling, and Paper Autopilot remain disabled.
- **Where to resume:** Obtain an explicit non-secret approval reference for migration `0009`, run the exact-target guarded migration command once, rerun `database-migration-readiness` and `daily-reconciliation-readiness`, then decide separately whether to authorize one guarded paper reconciliation.
...
- **Recommended next prompt:** `Approve migration 0009 with reference MIGRATION-0009-<your-ticket>` (replace the placeholder with your actual change/ticket reference).
