...
- **Verification:** 170 tests, typecheck, lint, production build, secret-surface audit, and diff checks pass. No Railway query or migration was run in this phase.
- **Next smallest unit:** Run the read-only readiness check against Railway, then review and explicitly approve applying migration `0009` if it is blocked only because the migration is not yet applied.
