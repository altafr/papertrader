...
- **Status:** Phase 3.12 opt-in shadow worker boundary and health record implemented; Railway migration and first operator-run reconciliation remain operational dependencies.
- **Current operating mode:** Paper only; order submission not yet enabled.
- **Current goal:** Wire the finalized-bar adapter and repository into the opt-in shadow command without enabling paper execution.
