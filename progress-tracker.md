...
### Phase 6.843 — scheduled cycle execution verification (2026-09-06)

- The hosted Railway research/risk scheduler completed its next 15-minute cycle at `2026-09-06T07:15:10Z` with status `completed` and one deterministic risk decision.
...
- Railway deployment `6f792520-c8a6-477c-a8ac-c339688c8617` reached `SUCCESS`.
- The durable `broker_entitlement_blocked` case now records `470` attempts and a redacted provider request ID in `last_error`, proving the diagnostic survives the database write path and restarts.
- No credentials or raw provider response bodies were stored; paper research, scheduling, and fail-closed position supervision remain active.
- **Next smallest unit:** give the latest request ID to Alpaca support, then rerun the exit and hosted contract after entitlement recovery.

### Phase 6.823 — stable Telegram failure cooldowns (2026-09-04)
...
### Phase 6.833 — evidence-gate ETA visibility (2026-09-06)

- Added an informational `estimatedReadyAt` field to the Worker evidence report and full-readiness output when fewer than 30 consecutive days are available.
...
- Railway deployment `c3779000-c755-4aa4-a48a-5b9500c656ce` reached `SUCCESS`.
- Live Worker health now reports the stable `crypto_order_entitlement_blocked` code plus a bounded Alpaca request ID (`3ae435e17191bd2e444e5ff620a54650`) for support escalation. The request ID is not a credential and no response body was exposed.
- Market data remains fresh, research remains scheduled, and the supervisor remains fail-closed with zero unmanaged positions.
- **Next smallest unit:** provide this request ID to Alpaca support/account administration, then rerun the deterministic crypto exit and hosted contract after permission is corrected.
...
- Paper order failures now retain a bounded Alpaca `X-Request-ID` in the server-side error record when supplied, enabling support investigation while response bodies and credentials remain redacted.
- Added regression coverage; 469 tests, typechecks, and lint pass. This does not retry, bypass, or alter any order/risk behavior.
- **Next smallest unit:** deploy the diagnostic revision and provide the resulting request ID to Alpaca support if the crypto entitlement remains blocked.
...
- Pushed the complete deployed `phase-6-10-operator-health` branch to `origin`, including the position-exit isolation, dashboard/Telegram failure visibility, tech_solver, and migration records.
- GitHub now preserves the same source history used for the successful Railway and Vercel deployments; no runtime or broker state changed.
- **Next smallest unit:** review Alpaca crypto order entitlement, then verify the first successful crypto exit against the deployed revision.

### Phase 6.814 — Telegram broker failure visibility deployed (2026-09-04)

- Railway deployment `2a6f4ad4-2b1c-4f3b-b7a6-a347d7899af4` reached `SUCCESS`.
- Live Worker health confirms the Telegram assistant is ready and the bounded `crypto_order_entitlement_blocked` code is available while market data remains fresh and research is scheduled.
- **Next smallest unit:** review Alpaca crypto order entitlement and verify a successful deterministic exit; no further observability deployment is required.
...
- The Vercel public-health parser now preserves the bounded Worker `positionManagement.failureCode`, and the authenticated dashboard displays it alongside Worker status.
- This makes the active `crypto_order_entitlement_blocked` condition visible in the operator UI without exposing provider response bodies or credentials. Web tests, workspace typechecks, and lint pass.
- **Next smallest unit:** deploy the web/API surface if required by the hosting pipeline, verify the dashboard displays the broker blocker, and rerun the full hosted contract after Alpaca entitlement recovery.

### Phase 6.810 — independent exit isolation deployed and verified (2026-09-04)

- Railway deployment `63990d56-cd99-44bf-a54d-8ee1356ea9d3` reached `SUCCESS`.
- Live health confirms the Worker still runs scheduled paper research with fresh market data and zero unmanaged positions; the crypto entitlement failure is explicitly classified and keeps the supervisor degraded.
- The runner now evaluates independent positions after a rejected exit and records each failed symbol for durable alerting before returning a degraded result. No broker order was created by this deployment verification.
- **Next smallest unit:** after Alpaca entitlement review, observe a successful crypto exit and confirm the Worker returns to healthy position management.
...
- The first lookback-backed hosted cycle produced a candidate, then failed safely because the hourly bar's nominal one-day expiry was already past at cycle creation.
- Risk-candidate construction now keeps expiry strictly after creation, using the original signal horizon when valid and a bounded one-hour minimum only for delayed/restarted cycles.
- Focused Worker tests (10) and typecheck pass; no broker or database state changed.
- **Next smallest unit:** deploy and verify a full scheduled cycle reaches deterministic risk evaluation without the expiry-construction failure.
...
- The hosted Worker completed the scheduled `23:45 UTC` crypto cycle at `2026-09-01T23:45:11.916Z`: `crypto_research` produced one BTC/USD candidate, deterministic risk approved it, and paper execution reconciled broker truth.
- The reconciled decision used quantity `0.02572240`, entry `77508.55950000`, and estimated stop-loss distance `4.99%`; stable intent `intent:BTC_USD:2026-09-01T23:00:00Z` was reused without duplicate-order or quantity-mismatch failures.
- The position-management pass kept BTCUSD protected (`shouldExit=false`, submitted exits `0`); all three positions remain covered and the Worker continues to receive fresh market data.
- **Next smallest unit:** continue unattended cycles and verify the next scheduled crypto run.

### Phase 6.786 — 23:30 UTC scheduled cycle evidence (2026-09-02)

- The hosted Worker completed the scheduled `23:30 UTC` crypto cycle at `2026-09-01T23:30:40.777Z`: `crypto_research` produced one BTC/USD candidate, deterministic risk approved it, and paper execution reconciled broker truth.
- The reconciled decision used quantity `0.02576732`, entry `77350.66000000`, and estimated stop-loss distance `4.99%`; stable intent `intent:BTC_USD:2026-09-01T23:00:00Z` was reused without duplicate-order or quantity-mismatch failures.
- The preceding position-management pass kept BTCUSD protected (`shouldExit=false`, submitted exits `0`), and Worker health remained healthy with fresh market data and zero unmanaged positions.
- **Next smallest unit:** continue unattended cycles and verify the next scheduled crypto run.
...
- The guarded Railway read-only migration plan completed successfully: the production `schema_migrations` table is present and `pending: []`.
- This confirms the deployed Worker and database schema are synchronized; no migration approval or deployment action is required for the current phase.
- The 23:15 UTC scheduled cycle remains verified with deterministic approval, paper reconciliation, fresh market data, and zero unmanaged positions.
- **Next smallest unit:** continue unattended paper cycles and re-audit the 30-day evidence counter at the next UTC boundary.

### Phase 6.783 — 23:15 UTC scheduled cycle evidence (2026-09-02)

- The hosted Worker completed the scheduled `23:15 UTC` crypto cycle at `2026-09-01T23:15:42.607Z`: `crypto_research` produced one BTC/USD candidate, deterministic risk approved it, and paper execution reconciled broker truth.
- The reconciled decision used quantity `0.02581235`, entry `77218.84900000`, and estimated stop-loss distance `4.99%`; stable intent `intent:BTC_USD:2026-09-01T23:00:00Z` was reused without duplicate-order or quantity-mismatch failures.
- The preceding position-management pass kept BTCUSD protected by its stored deterministic plan (`shouldExit=false`, submitted exits `0`); Worker health remained healthy with zero unmanaged positions and fresh market data.
- **Next smallest unit:** continue unattended cycles and verify the next scheduled crypto run.

### Phase 6.778 — 22:30 UTC scheduled cycle evidence (2026-09-02)

- The hosted Worker completed the scheduled `22:30 UTC` crypto cycle at `2026-09-01T22:30:03.906Z`: `crypto_research` produced one BTC/USD candidate, deterministic risk approved it, and paper execution reconciled broker truth.
- The reconciled decision used quantity `0.02587348`, entry `77023.62950000`, and estimated stop-loss distance `4.99%`; stable intent `intent:BTC_USD:2026-09-01T22:00:00Z` was reused without duplicate-order or quantity-mismatch failures.
- Worker health remained healthy with zero unmanaged positions and the next run advanced to `22:45 UTC`. No risk threshold or operating mode changed.
- **Next smallest unit:** continue unattended cycles and verify the next scheduled crypto run.

### Phase 6.779 — 22:45 UTC scheduled cycle evidence (2026-09-02)

- The hosted Worker completed the scheduled `22:45 UTC` crypto cycle at `2026-09-01T22:45:03.944Z`: `crypto_research` produced one BTC/USD candidate, deterministic risk approved it, and paper execution reconciled broker truth.
...
- The guarded Railway audit completed with no broker or database mutation. Runtime readiness, complete exit-plan coverage (`3` positions, `0` unmanaged), and verified Telegram delivery all remain ready.
- The sole release blocker remains `minimum_30_consecutive_calendar_days_not_met`: `10` calendar days, `9` consecutive days, and `21` days remaining of the required `30`. Current bounded paper metrics are final equity `99642.96`, total P/L `252.75`, and maximum drawdown `1.24%` across `9,144` snapshots.
...
### Phase 6.775 — 22:15 UTC scheduled cycle evidence (2026-09-02)

- The hosted Worker completed the scheduled `22:15 UTC` crypto cycle: `crypto_research` produced one BTC/USD candidate, deterministic risk approved it, and paper execution reconciled broker truth with quantity `0.02579908`, entry `77264.19600000`, and estimated stop loss `4.99%`.
- The stable intent advanced to `intent:BTC_USD:2026-09-01T22:00:00Z`; no retry, duplicate-order, or quantity-mismatch failure appeared. Worker health remained healthy and the next run is scheduled for `22:30 UTC`.
- **Next smallest unit:** continue unattended cycles and re-audit the evidence counter after the next UTC calendar boundary.
...
- Workspace ESLint passed with `--max-warnings=0` across `apps` and `packages`.
- No code, runtime configuration, or broker state changed; this confirms the current release remains clean while scheduled paper evidence accumulates.
...
### Phase 6.758 — Full regression and production build verification (2026-09-02)

- Full workspace regression passes: 99 test files, 455 tests.
...
- Infrastructure audit: PostgreSQL is reachable and the migration plan reports `pending:[]`; no schema action is required for the next evidence cycle.
- The 20:30 UTC research tick completed with one risk decision and zero approved entries; the scheduler advanced its next run to 20:45 UTC without degradation.
...
- Risk-cycle decision logs now include bounded entry price, quantity, and estimated loss percentage alongside approval status and rejection reasons, making the 2%/5% policy auditable without exposing credentials or model output.
- Focused Worker tests and typecheck pass; **Next smallest unit:** deploy Worker and verify the next scheduled risk-cycle log.
- Worker deployment completed successfully; hosted verification passes with healthy Worker/API, fresh market data, ready position management, zero unmanaged positions, and future schedulers. The next risk cycle will emit the new sizing telemetry.
- The first post-fix 20:45 UTC cycle approved and reconciled a BTC/USD paper trade with quantity `0.02577179`, entry `77353.32`, and estimated loss `4.99%`; the decision log records no rejection reasons and the position manager continues to monitor the resulting portfolio.
- Hosted verification remains passing; no live trading was enabled.
...
- Worker deployment is live; hosted verification passes with healthy Worker/API, fresh market data, ready position management, zero unmanaged positions, and the next research tick scheduled for 21:15 UTC. The 21:00 failure remains fail-closed and no order was authorized from it.
...
- Guarded market-close verifier passed with 3 persisted delivered `daily_portfolio_summary` events; latest captured at `2026-09-01T20:04:55Z`.
- Hosted runtime remains healthy with fresh market data, active schedulers, zero unmanaged positions, and verified Telegram delivery.
- Full readiness remains blocked only by the 30-day evidence window (9 consecutive days captured, 21 remaining).
- **Next smallest unit:** continue unattended paper cycles and re-audit after the next evidence-day boundary.

### Phase 6.729 — Live daily-summary protection verification (2026-09-02)

- Worker deployment `3659f24a-cc99-4423-895f-d1d0eafec0d1` succeeded.
- Hosted verification passes with healthy paper runtime, fresh market data, active schedulers, release consistency, and zero unmanaged positions.
- Future market-close summaries will include active ratcheted stops and targets alongside P/L and exposure.
- **Next smallest unit:** continue unattended paper cycles and verify the next persisted market-close summary.
...
- Re-ran hosted verification after the Telegram portfolio deployment; Worker/API are healthy, market data is fresh, schedulers are active, release identity is consistent, and unmanaged position count remains zero.
- Full autonomous readiness remains blocked only by the required evidence window (9 consecutive days captured, 21 remaining).
- **Next smallest unit:** continue unattended paper cycles and re-audit after the next evidence-day boundary.

### Phase 6.726 — Live Telegram portfolio verification (2026-09-02)

- Worker deployment `225365cc-ec0d-4575-9677-8e8c7a69737a` succeeded.
- Hosted verification passes with healthy Worker/API, fresh market data, active schedulers, release consistency, and zero unmanaged positions.
- Routine Telegram portfolio responses now expose active stop and target values for managed positions.
- **Next smallest unit:** continue unattended paper cycles and accumulate the evidence window.

### Phase 6.725 — Telegram portfolio risk visibility (2026-09-02)

- Routine Telegram portfolio responses now include each managed position’s active ratcheted stop and target, not just a managed/review label.
- This improves real-time operator awareness while remaining read-only; deterministic Worker position management remains the only exit authority.
- Focused assistant tests, Worker typecheck, and lint pass; no broker state changed.
- **Next smallest unit:** deploy the Worker revision and verify hosted health.

### Phase 6.724 — Durable ratchet runtime evidence (2026-09-02)

- Live PostgreSQL verification confirms the new trailing-stop field is populated for 2 managed submission records; the Worker has continued successful 60-second position-management passes.
- Worker health is healthy with fresh market data, scheduled research, zero unmanaged positions, and no active kill switch.
- Full readiness remains blocked only by the 30-day evidence gate (9 consecutive days captured, 21 remaining).
- **Next smallest unit:** continue unattended paper cycles and re-audit after the next evidence-day boundary.

### Phase 6.723 — Live Mini App active-stop verification (2026-09-02)

- API deployment `e14476e9-ac7d-477a-a0b2-df8a8aa360d8` and Vercel production deployment completed successfully.
- Hosted verification passes with healthy paper runtime, fresh data, active schedulers, release consistency, and zero unmanaged positions; the Mini App returns HTTP 200.
- Telegram portfolio data now carries canonical managed-position metadata, including active ratcheted stops and targets.
- **Next smallest unit:** continue unattended paper cycles and accumulate the evidence window.
...
- Worker deployment `23e28e5b-3376-459d-800f-12085b04afa8` succeeded.
- Hosted verification passes with healthy Worker/API, fresh market data, active schedulers, release consistency, and zero unmanaged positions.
- The read-only Telegram assistant now reports the effective ratcheted stop and original plan stop for managed positions.
- **Next smallest unit:** continue unattended paper cycles and accumulate the evidence window.

### Phase 6.720 — Telegram active-stop visibility (2026-09-02)

- Telegram exit-plan responses now show the effective ratcheted stop and original planned stop for each managed position.
- This keeps the read-only assistant aligned with the Worker and dashboard after a favorable move, without adding order authority.
- Focused assistant tests, Worker typecheck, and lint pass; no broker state changed.
- **Next smallest unit:** deploy the Worker revision and verify Telegram/hosted health.

### Phase 6.719 — Live canonical dashboard verification (2026-09-02)

- API deployment `7d8cce86-32de-47f5-94f1-7583cb71a2b0` and Vercel production deployment completed successfully.
- Hosted verification passes with healthy Worker/API, fresh market data, active schedulers, release consistency, and zero unmanaged positions; the Telegram Mini App returns HTTP 200.
- Canonical crypto symbol matching is now active on production dashboard projections.
- **Next smallest unit:** continue unattended paper cycles and accumulate the evidence window.
...
- Authenticated API position metadata now includes the durably ratcheted effective stop, and the dashboard displays it alongside the immutable planned stop.
- This makes the operator view match the stop actually enforced by the Worker after favorable moves and restarts.
- API/web typechecks, focused contract tests, and lint pass; no broker state changed.
- **Next smallest unit:** deploy API and web revisions, then verify the live dashboard and hosted runtime.
...
- Scheduled Paper Autopilot execution now selects broker-compatible order duration by asset class: `gtc` for crypto and `day` for US equities.
- Added regression coverage for both branches; focused Worker tests and typecheck pass.
- This closes a crypto execution-path defect without broadening order authority or changing risk limits; deterministic approval, bracket, reconciliation, and kill-switch gates remain required.
- **Next smallest unit:** deploy the Worker revision and verify hosted health and readiness.
...
### Phase 6.689 — Persisted Telegram delivery verification (2026-09-01)

- The full readiness command now treats Telegram delivery as verified only when a persisted `telegram_channel_test` event has status `sent`; configuration alone cannot clear this gate.
...
### Phase 6.710 — Risk-cycle position identity alignment (2026-09-02)

- Scheduled risk-cycle coverage checks now canonicalize broker symbols before determining whether existing positions have complete exit plans.
- This prevents `BTCUSD`/`BTC/USD` formatting differences from falsely blocking otherwise valid risk decisions; regression coverage and Worker typecheck pass.
- No order behavior was broadened: the deterministic risk engine, exit-plan requirement, and fail-closed gates remain unchanged.
- **Next smallest unit:** deploy the Worker revision and verify position coverage remains zero-unmanaged.
...
- Added a regression guard requiring the bounded evidence-history capacity to remain at least 30,000 snapshots, preventing future changes from making the 30-day gate unreachable.
- Worker performance tests and typecheck pass; this is test/documentation-only and does not require a deployment.
- **Next smallest unit:** continue unattended scheduled cycles and monitor evidence continuity.

### Phase 6.704 — Natural scheduled cycle verification (2026-09-02)

- Observed the Worker complete the natural 18:45 UTC research/risk cycle without manual triggering or deployment.
- Hosted health recorded `lastRiskCycleStatus=completed`, one deterministic risk decision, zero approvals, and advanced the next run to 19:00 UTC; position management remained ready with zero unmanaged positions.
- This verifies the continuous scheduler path is operating while preserving fail-closed selection behavior when no candidate is approved.
- **Next smallest unit:** continue unattended cycles and accumulate the remaining paper-evidence days.

### Phase 6.703 — Telegram delivery state alignment (2026-09-02)

- API operations health now checks PostgreSQL for any sent `telegram_channel_test` event and exposes `deliveryVerification=verified` when present.
- Dashboard parsing and presentation accept the verified state and no longer display a false “Delivery unverified” warning after a successful channel test.
- API/web typechecks and focused regression tests pass; this is read-only observability and does not alter alert delivery or order authority.
- **Next smallest unit:** deploy API and Vercel revisions, then verify the production health/readiness surfaces.
...
- Full-readiness now queries PostgreSQL directly for any sent `telegram_channel_test` event instead of scanning only the latest 100 alerts.
- This prevents long-running alert history from causing a false delivery block after the original verified test scrolls out of the recent window.
- Workspace regression remains green (99 files / 445 tests); DB and Worker typechecks pass. No broker or notification behavior changed.
- **Next smallest unit:** deploy and verify the hosted readiness report, then continue collecting evidence.

### Phase 6.699 — Evidence-window history capacity (2026-09-01)

- Increased the bounded account-snapshot history used by full readiness and Telegram evidence replies from 500 to 10,000 rows, enough for a 30-day paper cycle at the configured 15-minute cadence.
- Centralized the limit and added coverage through the performance/readiness/assistant test suite; 17 focused tests and Worker typecheck pass.
- This preserves fail-closed behavior while preventing the evidence query itself from making the 30-day gate impossible to satisfy.
- **Next smallest unit:** deploy and verify the hosted readiness report, then continue collecting evidence.

### Phase 6.698 — Two-decimal daily-summary formatting (2026-09-01)

- Daily Telegram market-close summaries now format equity, cash, and buying power to exactly two decimal places, matching P/L, exposure, and operator display conventions.
- Added regression coverage; Worker focused tests and typecheck pass.
- This is presentation-only: broker precision remains unchanged and no order, risk, exit-plan, scheduler, or reconciliation behavior changed.
- **Next smallest unit:** deploy and verify the Worker remains healthy, then continue accumulating the 30-day evidence window.
...
### Phase 6.670 — Broker-linked legacy exit-plan prefill (2026-09-01)

- The guarded legacy-position adoption command can now derive an omitted entry price from the weighted average of selected broker-reported filled buys.
...
### Phase 6.676 — Telegram exit-plan query routing (2026-09-01)

- Telegram questions about exit plans, stop loss, take profit, unmanaged positions, and position management now stay on the local read-only path instead of being misclassified as web research.
...
### Phase 6.680 — Hosted Telegram portfolio exit-plan visibility (2026-09-01)

- Railway deployed Worker commit `a612c4b` successfully.
- Post-deploy health confirms Telegram assistant readiness, Firecrawl configuration, fresh crypto data, and active position supervision; the only degraded condition remains the two unmanaged legacy positions.
- **Next smallest unit:** ask the connected Telegram assistant for a portfolio summary to verify the per-position exit-plan labels in the live channel.

### Phase 6.681 — Telegram portfolio P/L summary (2026-09-01)

- Telegram portfolio summaries now include aggregate day P/L and unrealized P/L, calculated with decimal-safe arithmetic and formatted to two decimals.
...
### Phase 6.683 — Continuous runtime safety evidence (2026-09-01)

- Live Worker health and logs show the scheduler advancing to its next run, crypto market data fresh, position management executing every 60 seconds, and AAPL receiving deterministic monitoring without an exit submission.
...
### Phase 6.687 — Hosted cross-surface Mini App verification (2026-09-01)

- Ran the verifier with Railway-held credentials and public API/Worker URLs; it returned HTTP 200 with 3 positions, 22 orders, 50 alerts, both P/L metrics, and matching unmanaged counts of 2.
- The result reported `surfaceConsistency=matched`; no credential values, broker orders, or database records were exposed or changed.
- **Next smallest unit:** keep the verifier in the deployment checklist and complete the operator Telegram `/dashboard` interaction plus legacy exit-plan approvals.
...
### Phase 6.641 — Signed Telegram Mini App activation (2026-08-31)

- Confirmed Railway API now contains the four required Telegram Mini App variables without exposing their values.
...
### Phase 6.643 — Telegram launch-path verification (2026-08-31)
...
### Phase 6.646 — Full regression and hosted readiness audit (2026-08-31)

- Full workspace regression passed: 97 test files / 431 tests, all typechecks, lint, and secret-surface audit.
- Hosted API reports healthy with the Telegram Mini App enabled/configured; Worker assistant is ready and market data is fresh.
- Overall Worker health remains degraded only for the two known unmanaged legacy positions; no broker or database state changed.
- **Next smallest unit:** send `/dashboard` in Telegram and tap **Open portfolio & alerts**, then complete operator-reviewed exit-plan remediation for BTCUSD and PFD.

### Phase 6.647 — Mini App configuration boundary hardening (2026-08-31)

- The API now requires the enable flag, bot token, operator user ID, and explicit origin before authenticating or serving Mini App data.
- Enabled deployments no longer fall back to wildcard CORS; incomplete configuration fails closed with HTTP 503.
- **Next smallest unit:** send `/dashboard` in Telegram and verify the signed Portfolio/Alerts view.

### Phase 6.648 — Mini App configuration hardening rollout (2026-08-31)

- Deployed the API configuration-boundary hardening revision to Railway.
...
### Phase 6.651 — Mini App recent-order view (2026-08-31)

- Added a bounded recent-orders list to the read-only Portfolio tab, including side, status, quantity, and update time.
- The list uses the already bounded API projection and does not add order or risk controls.
- **Next smallest unit:** deploy the frontend revision and verify positions, orders, P/L, and alerts from Telegram.

### Phase 6.652 — Mini App recent-order rollout (2026-08-31)
...
### Phase 6.654 — Mini App unmanaged-position visibility (2026-08-31)

- Added a bounded read-only unmanaged-position projection to the signed Mini App response using the same exit-plan completeness rule as the protected dashboard.
- The Mini App now flags positions requiring operator-reviewed exit-plan provenance; no order or risk controls were added.
- **Next smallest unit:** deploy API/frontend and verify the warning alongside live positions.

### Phase 6.655 — Mini App unmanaged-position rollout (2026-09-01)

- Pushed the implementation to GitHub and deployed the API revision to Railway.
- Hosted API health confirms the Mini App is enabled/configured and the pinned release is active; Vercel production serves `/telegram` with HTTP 200.
- The Mini App now receives the authoritative unmanaged-position warning for BTCUSD/PFD; no broker or database state changed.
- **Next smallest unit:** open `/dashboard` in Telegram and verify the warning, positions, recent orders, P/L, and alerts.

### Phase 6.656 — Signed Mini App payload verification (2026-09-01)

- Confirmed the latest Vercel deployment is ready and production `/telegram` remains available.
- Server-side signed verification returned HTTP 200 with 3 positions, 22 recent orders, 50 alerts, Day P/L, Unrealized P/L, and unmanaged symbols BTCUSD/PFD.
- No secrets were exposed and no broker/database state changed.
- **Next smallest unit:** open `/dashboard` in Telegram and tap **Open portfolio & alerts** for the interactive check.

### Phase 6.657 — Mini App freshness visibility (2026-09-01)

- Added a five-minute snapshot-freshness classifier and visible stale/unknown warning to the Mini App.
- Added regression coverage for fresh, stale, and invalid timestamps; this remains read-only and cannot authorize trading.
- **Next smallest unit:** publish the frontend revision and verify the freshness indicator in Telegram.

### Phase 6.658 — Freshness rollout verification (2026-09-01)

- Confirmed the Vercel production deployment is Ready and `/telegram` remains available.
...
### Phase 6.665 — Scheduled research cycle verification (2026-09-01)
...
### Phase 6.667 — Continuous cadence confirmation (2026-09-01)
